...
const Day10_2 = () => {
  ...
  const handleNext = async () => {
    ...
    const fetchData = async () => {
      await axios({
        method: "get",
        url: `/api/training/training-progress?employeeID=${cookies.employeeid}&day=10&part=2`,
        timeout: 5000, // 5 seconds timeout
        headers: {},
      }).then(async response => {
        ...
        if (result1.length == 0) {
          await axios({
            method: "post",
            url: `/api/training/email-sender-day10_2`,
            timeout: 5000, // 5 seconds timeout
            headers: {},
            data: {
              username: cookies.username,
          ...
          <Box sx={{ width: "100%" }}>
            <Grid container>
              ...
              <Grid item xs={4} style={{ height: "90vh", overflow: "auto" }}>
                ...
                >
                  <div style={{ padding: "1%" }}>
                    <h2 className="title-day">Day 10 Part 2</h2>
                    <h3
                      style={{
                        color: "#fcfaf8",
                        fontWeight: "400",
                    ...
                    >
                      ...
                      >
                        1003 . Create a Subcontractor Sub Mod and send it via
                        email.
                      </p>
                    </div>
                  </div>
